#!/usr/bin/env tsx
/**
 * Drift detection for Bolivian Law MCP.
 *
 * Checks if upstream Bolivian Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Bolivian Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'bolivian-law-mcp/1.0 drift-detect' },
      });
