#!/usr/bin/env tsx
/**
 * Database builder for Bolivian Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Bolivian Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Bolivian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'BO');
    insertMeta.run('source', 'lexivox.org');
    insertMeta.run('licence', 'Public domain (Bolivian legislation)');
  });
  writeMeta();
