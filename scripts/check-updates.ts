#!/usr/bin/env tsx
/**
 * Bolivian Law MCP — Data Freshness Checker
 *
 ...
 * 3. Source portal — verifies LexiVox is reachable
 *
 * Exit codes:
...
const PORTAL_URL = 'https://www.lexivox.org';
const PORTAL_NAME = 'Bolivian Law (LexiVox)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/bolivian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Bolivian Law MCP — Data Freshness Check');
